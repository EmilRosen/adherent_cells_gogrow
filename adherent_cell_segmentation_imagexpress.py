...
basePath = "/media/emiro593/AdherentCells/Hitesh_2021_10/";
rawPath = basePath + "Dasatinib 3013 and 3180/";

folderPattern  = "{celline}_{set:d}_{treatment}_{row}{col:02d}";
trackPattern   = folderPattern + ".csv";
filePattern    = "{frame:d}.{fileEnding}";

rawFolderPattern   = "{celline}_{set:d}";
rawWellPattern     = "{treatment}";
rawPattern         = "{row}{col:d} {frame:d}.{fileEnding}";
...
classes = [
	...
	{"name": "painted", "index": 2, "color": 0xFFFFFF},		# Cells
];
...
if __name__ == '__main__':
	...
	for parentInput in parentInputs:
		wellInputs = utils.getInputFiles(rawWellPattern, parentInput["path"] + "/" + parentInput["file"] + "/");

		for wellInput in wellInputs:
			if ".avi" in wellInput["file"]:
				continue;

			inputs = utils.getInputFiles(rawPattern, wellInput["path"] + "/" + wellInput["file"] + "/");

			for input in inputs:
				savePath = filterPath + folderPattern.format(**{**parentInput, **wellInput, **input}) + "/";
				utils.defineOutput(savePath);
				...
				if not os.path.isfile(saveFile):
					...
					res = cv2.resize(R, dsize=(1842, 1842), interpolation=cv2.INTER_CUBIC);
